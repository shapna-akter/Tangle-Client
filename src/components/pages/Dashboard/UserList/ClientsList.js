...
const ClientsList = () => {
  ...
  const {
    ...
    queryKey: ["clients", disableType],
    queryFn: async () => {
      let url = `https://server.tanglecare.us/api/v1/getUser?role=client`;
  ...
  const handleBlockOrUnblock = (clientId) => {
    fetch(`https://server.tanglecare.us/api/v1/users/disable/${clientId}`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
              ...
              ? // Render search results if available
                searchResults.map((client, i) => (
                  <tr key={client?._id}>
                    ...
                    <td>
                      {client?.name?.firstName} {client?.name?.middleName}{" "}
                      {client?.name?.lastName}
                    </td>
                    <td>{client?.email}</td>
                    <td>{client?.phoneNumber}</td>
                    <td>
                      {client?.address?.address1} , {client?.address?.address2}
                    </td>
                    ...
                    <td>
                      <input
                        type="checkbox"
                        className="toggle"
                        checked={client.disablesStatus}
                        onChange={() => handleBlockOrUnblock(client._id)}
                      />
                    </td>
                  </tr>
                ))
              : // Otherwise, render clients data
